...
async def alice(ctx: lightbulb.Context) -> None:
    ...
    miscutils.send_image(
        LOCALIZATION_DATA["channels"]["resources"], alice, ctx.get_guild()
    )
...
def send_clue(self, ctx, time: int):
    ...
    channel = miscutils.get_text_channels(ctx.game.guild)[channel]
...
async def assign_times(ctx: lightbulb.Context) -> None:
    ...
    while True:
        ...
        if _test_clue_buckets(ctx, clue_buckets):
            break
